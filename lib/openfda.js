...
// Recalls: 60-day lookback (matches app-wide "reasonable dates" window), US-only section (no EU equivalent configured).
export async function fetchRecalls() {
  ...
  for (const r of results) {
    ...
    rows.push({
      ...
      cfresId: r.cfres_id,
      url: r.cfres_id
        ? `https://www.accessdata.fda.gov/scripts/cdrh/cfdocs/cfRES/res.cfm?id=${r.cfres_id}`
        : null,
    });
  }
