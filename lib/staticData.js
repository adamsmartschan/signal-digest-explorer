...
// Add this to lib/staticData.js (anywhere after automotiveGroups is
// defined -- e.g. right after the locateHire() function). It maps a
// free-text company name, as it appears in the Google Sheet's "Company" /
// "Company Name" columns, to a tracked group id. Promotions and Open Roles
// are sheet-sourced and don't carry a groupId field themselves, so this is
// what lets the company master sheet filter them correctly.

const COMPANY_NAME_TO_GROUP = {
  "ktm ag": "ktm",
  "ktm": "ktm",
  "gasgas motorcycles gmbh": "ktm",
  "gasgas": "ktm",
  "husqvarna motorcycles gmbh": "ktm",
  "husqvarna motorcycles": "ktm",
  "bmw motorrad": "bmw",
  "bmw group": "bmw",
  "piaggio group": "piaggio",
  "piaggio": "piaggio",
  "vespa": "piaggio",
  "aprilia": "piaggio",
  "moto guzzi": "piaggio",
  "gilera": "piaggio",
  "ducati motor holding": "ducati",
  "ducati": "ducati",
};

export function groupIdForCompanyName(name) {
  if (!name) return null;
  const key = name.trim().toLowerCase();
  if (COMPANY_NAME_TO_GROUP[key]) return COMPANY_NAME_TO_GROUP[key];
  // Fall back to substring match for messier free-text values (e.g. "KTM
  // AG - Austria" or "Ducati Motor Holding S.p.A.")
  for (const [needle, groupId] of Object.entries(COMPANY_NAME_TO_GROUP)) {
    if (key.includes(needle)) return groupId;
  }
  return null;
}
